//! Runtime behaviour of `#[derive(CopOptions)]`-generated code.

use murphy_plugin_api::{ConfigErrorKind, CopOptions, MurphyCopOptionV1};
...
fn slice_str(slice: &murphy_plugin_api::MurphySlice) -> &str {
    if slice.len == 0 {
        return "";
    }
    let bytes = unsafe { std::slice::from_raw_parts(slice.ptr, slice.len) };
    std::str::from_utf8(bytes).expect("schema slice is UTF-8")
}
...
fn schema_describes_each_field_in_order() {
    let schema: &[MurphyCopOptionV1] = Opts::SCHEMA;
    ...
    assert_eq!(slice_str(&schema[1].ty), "enum");
