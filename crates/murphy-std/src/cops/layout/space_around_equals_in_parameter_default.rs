...
//!   `opposite_style_detected` calls (murphy-8zw3, murphy-ilrx) only feed
//!   `--auto-gen-config` TODO-file generation and cross-file style-ambiguity
//!   tracking — neither exists in Murphy — and never change the offenses or
//!   autocorrections for any given file. Murphy mirrors the upstream
//!   `check_optarg` shape via the no-op `Cx::correct_style_detected()` /
//!   `Cx::opposite_style_detected()` surface (murphy-ilrx; same precedent as
//!   `Layout/FirstParameterIndentation`'s discarded `_detected`): the calls are
//!   present for structural parity but intentionally persist no state.
//! ```
//!
//! ## Options
//!
...
impl SpaceAroundEqualsInParameterDefault {
    ...
    fn check_optarg(&self, node: NodeId, cx: &Cx<'_>) {
        ...
        // RuboCop's `ConfigurableEnforcedStyle` style-tracking
        // (`correct_style_detected` / `opposite_style_detected` via
        // `check_optarg` → `incorrect_style_detected`) only feeds
        // `--auto-gen-config` inference and never changes which offenses fire.
        // Murphy's surface is the no-op `Cx::correct_style_detected()` /
        // `Cx::opposite_style_detected()` (murphy-ilrx); the calls below mirror
        // the upstream shape without persisting state.
        match style {
