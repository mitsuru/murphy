...
//! `murphy-rails` (ADR 0038, design §5 in
//! `docs/plans/2026-05-22-plugin-reboot-design.md`).
//!
//! `murphy-rails` is a dynamic plugin pack distributed with Murphy.
//! Same shape as `murphy-example-pack/tests/dep_boundary.rs`: every
...
#[test]
fn murphy_rails_runtime_murphy_deps_match_allow_list() {
    ...
    let pkg = packages
        .iter()
        .find(|p| p["name"].as_str() == Some("murphy-rails"))
        .expect("`murphy-rails` package present in metadata");
    ...
    assert_eq!(
        ...
        "murphy-rails must depend only on `murphy-plugin-api` at runtime \
