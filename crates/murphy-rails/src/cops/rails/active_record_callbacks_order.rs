...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/ActiveRecordCallbacksOrder";
pub(crate) const MESSAGE_BYTES: &[u8] =
    b"`%<current>s` is supposed to appear before `%<previous>s`.";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 6] = [
        b"active_record_callbacks_order",
        b"ActiveRecordCallbacksOrder",
        b"active",
        b"record",
        b"callbacks",
        b"order",
