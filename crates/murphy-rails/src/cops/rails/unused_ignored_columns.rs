...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/UnusedIgnoredColumns";
pub(crate) const MESSAGE_BYTES: &[u8] =
    b"Remove `%<column_name>s` from `ignored_columns` because the column does not exist.";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 1] = [b"ignored_columns="];
