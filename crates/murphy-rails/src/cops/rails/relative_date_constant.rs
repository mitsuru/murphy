...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/RelativeDateConstant";
pub(crate) const MESSAGE_BYTES: &[u8] =
    b"Do not assign `%<method_name>s` to constants as it will be evaluated only once.";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 5] = [
        b"relative_date_constant",
        b"RelativeDateConstant",
        b"relative",
        b"date",
        b"constant",
    ];
