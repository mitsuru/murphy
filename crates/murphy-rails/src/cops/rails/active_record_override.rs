...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/ActiveRecordOverride";
pub(crate) const MESSAGE_BYTES: &[u8] =
    b"Use %<prefer>s callbacks instead of overriding the Active Record method `%<bad>s`.";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 5] = [
        b"active_record_override",
        b"ActiveRecordOverride",
        b"active",
        b"record",
        b"override",
    ];
