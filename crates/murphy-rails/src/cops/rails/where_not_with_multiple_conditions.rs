...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/WhereNotWithMultipleConditions";
pub(crate) const MESSAGE_BYTES: &[u8] =
    b"Use a SQL statement instead of `where.not` with multiple conditions.";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 2] = [b"where.not(", b"where_not_with_multiple_conditions"];
