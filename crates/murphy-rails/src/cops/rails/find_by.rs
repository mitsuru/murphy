...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/FindBy";
pub(crate) const MESSAGE_BYTES: &[u8] = b"Use `find_by` instead of `where%<dot>s%<method>s`.";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 2] = [b"first", b"take"];
