...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/RequireDependency";
pub(crate) const MESSAGE_BYTES: &[u8] = b"Do not use `require_dependency` with Zeitwerk mode.";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 1] = [b"require_dependency"];
