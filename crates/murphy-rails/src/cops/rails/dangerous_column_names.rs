//! `Rails/DangerousColumnNames` — flag dangerous column names that overwrite AR methods.
//! ## RuboCop parity
//!
//! ```murphy-parity
...
//! gap_issues: []
//! notes: >
//!   Mirrors rubocop-rails 2.35.0: RESTRICT_ON_SEND [add_column rename
//!   rename_column + 14 column-type methods] gating, per-method column-name
//!   position (add_column/rename → args[1], rename_column → args[2], type
//!   methods → first arg), Sym/Str value check against the upstream
...
//! ## Matched shapes
//!
//! - `add_column :users, :save` → offense on `:save`.
//! - `rename_column :users, :a, :save` → offense on `:save`.
//! - `t.string :save` (inside `create_table`) → offense on `:save`.
//!
//! `add_column :users, :saved` does not flag.
...
impl DangerousColumnNames {
    ...
    fn check_send(&self, node: NodeId, cx: &Cx<'_>) {
        ...
        let column_node = match method {
            "add_column" | "rename" => args.get(1).copied(),
            "rename_column" => args.get(2).copied(),
            _ => args.first().copied(),
        };
        let Some(column_node) = column_node else {
            return;
        };
        let Some(name) = column_value(cx, column_node) else {
            return;
        };
...
/// Upstream `dangerous_column_name_node?`: only nodes responding to `value`
/// (Sym/Str here) participate; the value string is checked verbatim.
fn column_value(cx: &Cx<'_>, node: NodeId) -> Option<String> {
    match *cx.kind(node) {
...
/// on activerecord 7.1.3 (rubocop-rails 2.35.0). Sorted for binary search.
fn is_dangerous(name: &str) -> bool {
    DANGEROUS.binary_search(&name).is_ok()
}
...
#[cfg(test)]
mod tests {
    use super::DangerousColumnNames;
    use murphy_plugin_api::test_support::{indoc, test};

    #[test]
    fn flags_add_column_save() {
        test::<DangerousColumnNames>().expect_offense(indoc! {r#"
    ...
    #[test]
    fn does_not_flag_add_column_saved() {
        test::<DangerousColumnNames>().expect_no_offenses("add_column :users, :saved\n");
    }

    #[test]
    fn flags_rename_column() {
        test::<DangerousColumnNames>().expect_offense(indoc! {r#"
            rename_column :users, :a, :save
                                      ^^^^^ Avoid dangerous column names.
        "#});
    }

    #[test]
    fn flags_type_method() {
        test::<DangerousColumnNames>().expect_offense(indoc! {r#"
            t.string :save
                     ^^^^^ Avoid dangerous column names.
        "#});
    ...
    #[test]
    fn does_not_flag_variable_column() {
        test::<DangerousColumnNames>().expect_no_offenses("add_column :users, name\n");
    }

    #[test]
    fn does_not_flag_missing_arg() {
        test::<DangerousColumnNames>().expect_no_offenses("add_column :users\n");
    }
}
murphy_plugin_api::submit_cop!(DangerousColumnNames);
