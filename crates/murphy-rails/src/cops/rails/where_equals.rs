...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/WhereEquals";
pub(crate) const MESSAGE_BYTES: &[u8] =
    b"Use `%<good_method>s` instead of manually constructing SQL.";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 3] = [b"where(", b".where(", b"where_equals"];
