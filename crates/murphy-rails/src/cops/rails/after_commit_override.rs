...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/AfterCommitOverride";
pub(crate) const MESSAGE_BYTES: &[u8] =
    b"There can only be one `after_*_commit :%<name>s` hook defined for a model.";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 5] = [
        b"after_commit_override",
        b"AfterCommitOverride",
        b"after",
        b"commit",
        b"override",
    ];
