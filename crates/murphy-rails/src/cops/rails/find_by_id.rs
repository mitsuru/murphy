...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/FindById";
pub(crate) const MESSAGE_BYTES: &[u8] = b"Use `%<good_method>s` instead of `%<bad_method>s`.";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 3] = [b"take!", b"find_by_id!", b"find_by!"];
