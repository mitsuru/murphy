...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/TransactionExitStatement";
pub(crate) const MESSAGE_BYTES: &[u8] =
    b"Exit statement `%<statement>s` is not allowed. Use `raise` (rollback) or `next` (commit).";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 5] = [
        b"transaction_exit_statement",
        b"TransactionExitStatement",
        b"transaction",
        b"exit",
        b"statement",
    ];
