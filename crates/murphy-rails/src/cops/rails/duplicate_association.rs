...
pub(crate) const NAME_BYTES: &[u8] = b"Rails/DuplicateAssociation";
pub(crate) const MESSAGE_BYTES: &[u8] =
    b"Association `%<name>s` is defined multiple times. Don't repeat associations.";
...
pub(crate) unsafe extern "C" fn run(
    ...
    let patterns: [&[u8]; 4] = [
        b"duplicate_association",
        b"DuplicateAssociation",
        b"duplicate",
        b"association",
    ];
