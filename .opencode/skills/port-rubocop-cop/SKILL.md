...
description: Use when asked to port a RuboCop, RuboCop-RSpec, or RuboCop-Rails cop to Murphy, add an RSpec/Rails/Style/Layout cop to murphy-* crates, translate a RuboCop rule into a Murphy plugin cop, or fill a parity gap in an already-implemented cop (e.g. "fill the gap in", "close the parity gap for", "murphy-xxxx の gap を埋めて").
---
...
> field, or autocorrect logic to the existing `impl` block. Also update the
> existing `murphy-parity` block: drop the fixed beads ID from `gap_issues`
> once the work is code-complete, update `notes` to describe what changed, but
> do **not** set `status: verified` yet — that happens in Phase 3 after the
> scoped re-audit.
...
Run `roborev-refine` against the worktree branch. In opencode, load the
`roborev-refine` skill when available and follow its review → fix →
re-review loop until the daemon reports passing or hits the iteration
cap.

Stay in the same worktree while the loop runs. If the skill is not
available in the current opencode installation, perform the same loop
manually: request review, fix unresolved findings, re-request review,
and stop only when review passes or the remaining findings need user
escalation.
...
5. **Clean up.** Use the `using-git-worktrees` skill to leave and clean
   up the worktree so the next port starts from a clean main.
