...
**Status: Phase 3 — user mruby cops complete.** `crates/murphy-core` + `crates/murphy-cli` build a working `murphy lint` that takes file args, directory args, or zero path args (discover from cwd via discovery-only `murphy.toml` `[files] include`/`exclude` + `.murphyignore`; `.gitignore` deliberately not honored), lints **file-level parallel across all cores** with deterministic output, and parses byte-identical files once per run (in-run memoization only — no persistent cache). User cops dropped as `.rb` into `cops/` (cwd-relative, ADR 0004) run **in-process via embedded mruby**, per-cop isolated (own `mrb_state`) + deadline-guarded + exception-isolated (a raising/runaway cop → one `severity:"error"` offense for that cop×file, run continues), reading the live shared prism AST through native primitives (no serialization), merged with the native cops into one deterministic array. The `fix` block is **captured-not-applied** (soft-(a)) — NO `autocorrect` field/output; autocorrect application + the `Offense.autocorrect` contract are Phase 4. The offense-JSON contract, exit codes, and TDD/snapshot harness are **frozen** (ADR 0006) and Phase 3 preserved them unchanged (ADR 0007); Phase 3 added cross-engine severity-precedence collision resolution (Error > Warning on a 4-tuple collision, ADR 0011) with the JSON shape unchanged, and is gated by ADR 0012. Phase 4+ build on this without renegotiating. The authoritative design — architecture, locked decisions, rejected alternatives with rationale — lives in `docs/plans/2026-05-19-murphy-design.md`. The phased implementation plan is `docs/plans/2026-05-19-murphy-implementation-plan.md`. Resolved Phase-0/gate decisions are ADRs in `docs/decisions/` (read these before Phase 2 / Phase 3 — they carry load-bearing constraints). Spike PoCs under `spikes/` are throwaway and are NOT promoted into `crates/`.
...
cargo test --workspace                            # full suite (Phase 3: 77 tests, all pass)
