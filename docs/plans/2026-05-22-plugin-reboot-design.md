...
- **動的 (`cdylib` build または `plugin-dynamic` feature 有効):**
  `register_cops!` は `#[no_mangle] extern "C" fn murphy_plugin_register` を
  emit する。`murphy-cli` は `dlopen` した `.so` から
  `dlsym("murphy_plugin_register")` を取得して呼ぶ。
- **静的 (`rlib`、`plugin-dynamic` feature 無効):** `register_cops!` は
...
path のみロードする(暗黙の探索パスは持たない)。相対 path は
`murphy.toml` のあるディレクトリを基準に解決。ロード失敗
(file not found / symbol missing / ABI version mismatch / load 中の
panic) は具体的 diagnostic とともに exit code 2 で停止。プラグインは
ADR 0004 に従い **trusted native code** として扱う(サンドボックスなし、
host 権限で動作) ── template repo / config 例 / load diagnostic は
この前提をユーザーに明示する文言を含める。
...
- §12 を 4 サブタスク(12a–12d)に分割し、それぞれ独立完了条件を持つ:
  ...
  - **12c 完了条件:** 未移植標準 cop が disabled registry に登録され
    ...
    subcommand(仮称 `murphy plugins list` ── 具体名は本タスクで確定)
    で disabled cop が `disabled: arena migration in progress` 注記
    付きで一覧表示される。ユーザーの既存 `murphy.toml` で disabled 中の
    cop が明示的に `enabled = true` 指定されていた場合は **warning を出
    して skip**(エラーにはしない ── 設定互換性のため)。
  - **12d 完了条件:** **各 namespace から最低 1 個ずつ**(Murphy / Lint
    / Style / Layout の 4 namespace、計 4 個以上)の代表 cop が arena 上
    で再実装され、既存スナップショットテストが green。選定基準は
    「namespace ごとに dispatch パターンの異なる cop を選ぶ」(典型的
    container 系・call 系・literal 系をカバー)。残りは 12c で disabled。
...
- **無効化中 cop の明示有効化:** ユーザーの `murphy.toml` で
  ...
  への参照を含める。
- **無効化された標準 cop の可視化:** disabled registry の内容は CLI
  subcommand(具体名は 12c で確定 ── 仮称 `murphy plugins list`)で
  `disabled: arena migration in progress` 注記付きで一覧表示される。
