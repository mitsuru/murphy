...
**Panic 境界の契約。** `extern "C"` 境界を Rust の panic が unwind すると
abort または UB になる。`register_cops!(mode = dynamic, ...)` が生成する
`#[no_mangle] extern "C" fn murphy_plugin_register` 内では
`std::panic::catch_unwind` で panic を捕捉し、新規追加する `MurphyPluginV1`
の error status (例 `register_status: u32`、`0 = ok`、`1 = panic`、
`2 = abi mismatch` 等) として host に返す。`murphy-cli` はこの status を
「plugin load failure」の 1 種として診断蓄積し、後段の集約処理(下記)で
報告する。`extern "C-unwind"` は採用しない ── Rust toolchain version
要件が増え、かつ host 側で `catch_unwind` の経路は結局必要になるため。
...
14a Rails 131 cop の disabled registry 一括登録
    (cop name / namespace / default metadata を構造化して登録 ──
    既存 `[cops.rules."Rails/..."]` が unknown-cop error にならない)  ← 14
...
- **murphy-rails の扱い:** §14 で `run_file` が撤廃される時点で
  text-matching 経路は失われる。murphy-rails の 131 cop は **§14a で
  ...
  - **§14a 完了条件:** Rails 131 cop の name / namespace / default
    metadata を構造化したテーブルが disabled registry に登録される。
    Rails 設定 fixture (`[cops.rules."Rails/..."]` を持つ `murphy.toml`)
    で `murphy lint` がエラーにならない。`murphy cops list --format
    json` の snapshot で Rails cop が `disabled: arena migration` と
    して列挙される。
  - **既知の退行 (known regression):** reboot 完了時点で Rails ユーザー
    は **lint coverage が一時的に大きく落ちる**(131 cop すべて未実行)。
    これは reboot epic の non-goal として明示する。au8 完了までの期間と
    リリースノート要件は別途リリース管理側で扱う(本 PRD のスコープ外)。
