...
- `Union`/`Not`/`Descend` 内の Node パターンでの `List` スロット使用(v1 は
  固定スロットのみ。`!(array $_ ...)` 等は `compile_error`)。
- `Sym` スロットおよびリテラル値の capture(非ノード capture)。
- atom の `(int 5)` ノードマッチ形式。
- v1 未対応 NodeKind(`rescue` 等 ~25 個以外)のスキーマ追加。
- スキーマが省略した NodeKind フィールド(`Case.else_` / `When.body` / `Def.receiver`
  / `While.post` / `Until.post`)は v1 ではパターンから参照できない(`List` スロットは
  末尾 1 個まで、の規約による)。
